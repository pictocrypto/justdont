...
import { Q as QueryClientProviderWithDefault, n as neverPersist, r as requiredParamInvariant } from './required-param-3ab382d0.esm.js';
import { useQuery, useQueryClient, useMutation } from '@tanstack/react-query';
import { constants, BigNumber, utils } from 'ethers';
import invariant from 'tiny-invariant';
import { u as useConnect } from './useConnect-54fe48c2.esm.js';
...
const WrappedThirdwebSDKProvider = _ref => {
  ...
  useEffect(() => {
    ...
    const _sdk = new ThirdwebSDK(provider, sdkOptions, storageInterface); // if we already have a signer from the wallet context, use it immediately
    ...
    setSDK(_sdk); // explicitly *not* passing the signer, if we have it we use it if we don't we don't
    // eslint-disable-next-line react-hooks/exhaustive-deps
...
function useUnclaimedNFTs(contract, queryParams) {
  ...
  return useQueryWithNetwork(cacheKeys.contract.nft.drop.getAllUnclaimed(contractAddress, queryParams), () => {
    requiredParamInvariant(contract, "No Contract instance provided"); // TODO make this work for custom contracts (needs ABI change)
...
function useAcceptDirectListingOffer(contract) {
  ...
  return useMutation(async data => {
    invariant(walletAddress, "no wallet connected, cannot make bid");
    requiredParamInvariant(contract === null || contract === void 0 ? void 0 : contract.direct, "No Direct instance provided");
...
function useExecuteAuctionSale(contract) {
  ...
  return useMutation(async data => {
    invariant(walletAddress, "no wallet connected, cannot make bid");
    requiredParamInvariant(contract === null || contract === void 0 ? void 0 : contract.auction, "No Auction marketplace instance provided");
...
function useOffers(contract, listingId) {
  var _result$data, _result$data$filter;

  const result = useContractEvents(contract, "NewOffer");
  return { ...result,
    data: (_result$data = result.data) === null || _result$data === void 0 ? void 0 : (_result$data$filter = _result$data.filter(ev => ev.data.listingId.eq(listingId))) === null || _result$data$filter === void 0 ? void 0 : _result$data$filter.map(ev => ev.data)
  };
}
/**
...
function useActiveClaimConditionForWallet(contract, walletAddress, tokenId) {
  ...
  return useQueryWithNetwork(cacheKeys.extensions.claimConditions.useActiveClaimConditionForWallet(contractAddress, walletAddress || "_NO_WALLET_", tokenId), async () => {
    ...
    const normalizedPrize = price ? utils.parseUnits(price, currencyMetadata.decimals) : null;
    const priceWithOverride = normalizedPrize || activeGeneralClaimCondition.price;
    const maxClaimableWithOverride = // have to transform this the same way that claim conditions do it in SDK
    ...
    return { // inherit the entire claim condition
...
function useDisconnect(options) {
  ...
  return async () => {
    var _data$data, _data$data$connector;

    // if there is a previous connector get it
    const previousConnector = (data === null || data === void 0 ? void 0 : (_data$data = data.data) === null || _data$data === void 0 ? void 0 : (_data$data$connector = _data$data.connector) === null || _data$data$connector === void 0 ? void 0 : _data$data$connector.previousConnector) || undefined; // if it's gnosis, just connect the previous connector
